...
const Pin = ({ pin: { postedBy, image, _id, destination, save } }) => {
  ...
  const alreadySaved = !!save?.filter(
    (item) => item.postedBy._id === user.gooleId
  )?.length;
          ...
          <div className="flex justify-between items-center gap-2 w-full">
            {destination && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-white flex items-cener gap-2 text-black font-bold p-2 pl-4 pr-4 rounded-full opacity-70 hover:shadow-md"
              >
